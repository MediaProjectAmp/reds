...
var types = {
  intersect:'zinterstore', union:'zunionstore', and:'zinterstore', or:'zunionstore'
};
...
exports.createSearch = function (key, redisClient) {
  if (!key) {
    throw new Error('createSearch() requires a redis key for namespacing');
  }
  if (!redisClient) {
    throw new Error('createSearch() requires a redisClient');
  }
  exports.client = redisClient;
  return new Search(key, redisClient);
};
...
/**
 * sets a set which the search results will be zintersected with
 * @param filterKey
 */
Query.prototype.addFilterKey = function (filterKey) {
...
Query.prototype.end = function (fn) {
  ...
  var rIdx = 1;
  if (this.filterKey) {
    if (type == 'zunionstore' && this.filterKey) {
      rIdx=2;
      var tkey2 = tkey + '2';
      cmds = [
        [type, tkey, keys.length, keys],
        ['zinterstore', tkey2, 2, [tkey, this.filterKey]],
        ['zrevrange', tkey2, 0, -1],
        ['zremrangebyrank', tkey2, 0, -1],
        ['zremrangebyrank', tkey, 0, -1]
      ]
    }
    if (type == 'zinterstore' && this.filterKey) {
      keys.push(this.filterKey)
      cmds = [
        [type, tkey, keys.length, keys],
        ['zrevrange', tkey, 0, -1],
        ['zremrangebyrank', tkey, 0, -1],
  ...
  db.multi(cmds).exec(function (err, ids) {
    ids = ids[rIdx];
    fn(err, ids);
  });
...
function Search(key, redisClient) {
  this.key = key;
  this.client = redisClient || exports.createClient();
}
